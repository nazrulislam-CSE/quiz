                ...
                <div class="card">
                    <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                        <p class="card-title my-0">{{ $pageTitle ?? 'Page Title' }} <span class="badge bg-danger side-badge"
                                style="font-size:17px;">{{ count($requests) }}</span> </p>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <div class="btn-group" role="group" aria-label="status-filter">
                                ...
                                <a href="{{ route('admin.withdraw.request.index', array_merge(request()->except('page'), ['status' => 'pending'])) }}"
                                    class="btn btn-sm {{ $status === 'pending' ? 'btn-warning text-white' : 'btn-outline-warning' }}">
                                    Pending
                                </a>

                                <a href="{{ route('admin.withdraw.request.index', array_merge(request()->except('page'), ['status' => 'approved'])) }}"
                                    class="btn btn-sm {{ $status === 'approved' ? 'btn-success text-white' : 'btn-outline-success' }}">
                                    Approved
                                </a>

                                <a href="{{ route('admin.withdraw.request.index', array_merge(request()->except('page'), ['status' => 'rejected'])) }}"
                                ...
                                <tbody>
                                    @forelse ($requests as $key => $request)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('admin.withdraw.request.show', $request->id) }}"
                                                    class="btn btn-success btn-sm mr-2"><i class="fas fa-eye"></i></a>
                                                <a href="{{ route('admin.withdraw.request.edit', $request->id) }}"
                                                    class="btn btn-primary btn-sm mr-2"><i class="fas fa-edit"></i></a>
                                                <a href="{{ route('admin.withdraw.request.delete', $request->id) }}"
