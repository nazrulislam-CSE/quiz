                ...
                <div class="card-body">
                    ...
                    <div class="tab-content">
                                        ...
                                        <select name="model_test_id" id="model_test_id" class="form-control">
                                            ...
                                            @if(old('model_test_id'))
                                                @foreach($models ?? [] as $model)
                                                    <option value="{{ $model->id }}" {{ old('model_test_id') == $model->id ? 'selected' : '' }}>{{ $model->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                        ...
                        <div class="tab-pane fade" id="excel-import" role="tabpanel">
                            <form action="{{ route('admin.model.mcq.import') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    {{-- select admission --}}
                                    <div class="form-group col-xl-4 col-lg-4 col-md-4">
                                    ...
                                    {{-- select model test --}}
                                    <div class="form-group col-xl-4 col-lg-4 col-md-4">
                                        <label for="import_model_test_id">Model Test: <span class="text-danger">*</span></label>
                                        @error('model_test_id') <span class="text-danger">{{ $message }}</span> @enderror
                                        <select name="model_test_id" id="import_model_test_id" class="form-control" required>
                                            <option value="">Select Model Test</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-xl-8 col-lg-8 col-md-8">
                                        ...
                                        <small class="text-muted d-block mt-2">
                                            ...
                                            <br>
                                            <a href="{{ route('admin.model.mcq.download-sample') }}" class="btn btn-sm btn-info mt-2">
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#import_admission_id').on('change', function () {
                ...
                $('#import_model_test_id').html('<option value="">Loading...</option>');
                if (admissionID) {
                    $.ajax({
                        url: "{{ url('/admin/model/mcq/get-departments') }}/" + admissionID,
                        type: "GET",
                    ...
                    $('#import_model_test_id').html('<option value="">Select Model Test</option>');
                }
            });
                ...
                if (groupID) {
                    $.ajax({
                        ...
                        success: function (data) {
                            $('#import_model_test_id').html('<option value="">Select Model Test</option>');
                            $.each(data, function (key, value) {
                                $('#import_model_test_id').append('<option value="' + value.id + '">' + value.name + '</option>');
                            });
                        }
                    });
                } else {
