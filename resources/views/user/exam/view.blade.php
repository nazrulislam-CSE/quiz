    ...
    <div class="container my-4">
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped align-middle">
                        @php
                            $examType = App\Models\Admission::find($examResult->admission->id);
                        @endphp
                        <tbody>
                            @if ($examType->name == 'ভার্সিটি এডমিশন')
                                <tr>
                                    <th class="bg-light w-25"><i class="fas fa-id-badge me-2 text-success"></i>Admission
                                    </th>
                            ...
                            @elseif($examType->name == 'পেপার ফাইনাল এক্সাম')
                                ...
                                <tr>
                                    <th class="bg-light"><i class="fas fa-building me-2 text-success"></i>Group</th>
                                    <td>{{ $examResult->group->name ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light"><i class="fas fa-book-open me-2 text-success"></i>Subject</th>
                                    <td>{{ $examResult->subject->name ?? '-' }}</td>
                                ...
                                <tr>
                                    <th class="bg-light"><i class="fas fa-tags me-2 text-success"></i>Paper Final</th>
                                    <td>{{ $examResult->paperFinal->name ?? '-' }}</td>
                                </tr>
                            @elseif($examType->name == 'ফাইনাল মডেল টেস্ট এক্সাম')
                                ...
                                <tr>
                                    <th class="bg-light"><i class="fas fa-tags me-2 text-success"></i>Model Test</th>
                                    <td>{{ $examResult->modelTest->name ?? '-' }}</td>
                                </tr>
                            @else
                                <tr>
                                    <th class="bg-light w-25"><i class="fas fa-id-badge me-2 text-success"></i>Admission
        ...
        <div class="card shadow-lg border-primary mb-4 mt-4">
            <div class="card-header bg-primary text-white fw-bold">
                @if ($examType->name == 'ভার্সিটি এডমিশন')
                    <i class="fas fa-tags me-2"></i> {{ $examResult->topic->name ?? '' }}
                @elseif($examType->name == 'পেপার ফাইনাল এক্সাম')
                    <i class="fas fa-tags me-2"></i> {{ $examResult->paperFinal->name ?? '' }}
                @elseif($examType->name == 'ফাইনাল মডেল টেস্ট এক্সাম')
                    <i class="fas fa-tags me-2"></i> {{ $examResult->modelTest->name ?? '' }}
                @else
                    <i class="fas fa-tags me-2"></i> {{ $examResult->topic->name ?? '' }}
                @endif
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <ul style="padding-left:18px;">
                                            @foreach ($mcq->answers as $answer)
                                                <li
                                                    @if ($answer->is_correct) style="color:green;font-weight:bold;" @endif>
                                                    {{ $answer->answer }}
