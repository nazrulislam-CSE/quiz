    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->status === 'approved') {
            ...
            if ($user && $user->refer_by) {
                ...
                // dd($directRef);
                if ($directRef) {
                  $directRef->increment('direct_commission', intval(($amount * 20) / 100));
                }
                ...
                if ($directRef && $directRef->refer_by) {
                    ...
                    if ($firstGen) {
                       $firstGen->increment('first_gen_commission', intval(($amount * 10) / 100));
                    }
                    ...
                    if ($firstGen && $firstGen->refer_by) {
                        ...
                        if ($secondGen) {
                           $secondGen->increment('second_gen_commission', intval(($amount * 5) / 100));
                        }
                    }
                }
            }
            $user->visa_amount += intval(($amount * 20) / 100);
            $user->save();
        }
