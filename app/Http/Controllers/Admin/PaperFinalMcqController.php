...
class PaperFinalMcqController extends Controller
{
    ...
    {
        $pageTitle = 'Paper Final MCQ List';

        $mcqs = Mcq::with(['admission', 'department','group', 'subject', 'paperFinal'])
        ->where('mcq_type', 3)
        ->select(
            'admission_id',
            'department_id',
        ...
        ->groupBy('admission_id', 'department_id','group_id', 'subject_id', 'paper_final_id')
        ->orderBy('latest_created_at', 'desc') // ✅ sort by aggregated column
        ->get();

        return view('admin.paper.mcq.index', compact('mcqs','pageTitle'));
    }
    ...
    {
        $pageTitle = 'Create Paper Final MCQ';
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.paper.mcq.index')->with('success', 'MCQs saved successfully!');
     ...
     */
    public function show($admission_id, $department_id, $group_id, $subject_id, $paper_final_id)
    {
        $pageTitle = 'MCQ Questions Details';
        
        $questions = Mcq::with(['admission', 'department', 'group', 'subject', 'paperFinal','answers'])
            ->where('admission_id', $admission_id)
            ->where('department_id', $department_id)
            ->where('group_id', $group_id)
            ->where('subject_id', $subject_id)
            ->where('paper_final_id', $paper_final_id)
            ->where('mcq_type', 3)
            ->latest()
            ->get();
            
        $topicInfo = $questions->first();
        return view('admin.paper.mcq.show', compact('questions', 'pageTitle', 'topicInfo'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit($admission_id, $department_id,$group_id, $subject_id, $paper_final_id)
    {
        $pageTitle = 'Edit Paper Final MCQ Questions';
        ...
        return view('admin.paper.mcq.edit', compact(
            'questions', 
            'topicInfo', 
            'pageTitle',
            'admissions',
     ...
     */
    public function update(Request $request, $admission_id, $department_id, $group_id, $subject_id, $paper_final_id)
    {
        DB::beginTransaction();
            ...
            if ($request->has('new_questions')) {
                ...
                foreach ($request->new_questions as $newQuestion) {
                    ...
                    $mcq = Mcq::create([
                        ...
                        'paper_final_id' => $paper_final_id,
                        'question' => $newQuestion['text'],
                        'correct_answer' => $newQuestion['correct_answer'],
                        'mcq_type' => 3, // Paper Final MCQ
                    ]);
     ...
     */
    public function destroy($admission, $department,$group, $subject, $paper)
    {
        $questions = Mcq::where('admission_id', $admission)
            ->where('department_id', $department)
            ->where('group_id', $group)
            ->where('subject_id', $subject)
            ->where('paper_final_id', $paper)
            ->get();
    ...
    public function getPapers($subject_id)
    {
        $papers = PaperFinal::where('subject_id', $subject_id)->where('status', '1')->get();
